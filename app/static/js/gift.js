document.addEventListener('DOMContentLoaded', function () {
  ...
  giftSubmitBtn.addEventListener('click', function () {
    ...
    // 서버로 사용자 존재 여부 확인 요청 보내기
    fetch(`/gifts/users/check?email=${receiverEmail}`)
      ...
      .then(data => {
        if (data.exists) {
          ...
          alert('받는 사람의 이메일이 올바르지 않습니다.');
        }
      })
      .catch(error => {
        console.error('사용자 존재 여부 확인 요청 실패:', error);
        alert('사용자 존재 여부 확인에 실패했습니다. 다시 시도해주세요.');
      });
  });
});